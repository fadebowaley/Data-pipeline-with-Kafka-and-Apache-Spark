...
def update(x):
    ...
    # x = (float(values["TimeStamp"]["$date"]) / 1000.0)
    x = (datetime.datetime.strptime((values["TimeStamp"]["$date"]), "%Y-%m-%dT%H:%M:%S.%fZ").timestamp())/1000.0
    # x=((values["TimeStamp"]["$date"])/1000.0)
    dt = datetime.datetime.fromtimestamp(x, tz)
...
p = figure(title="Water Temperature Sensor Data", x_axis_type="datetime", sizing_mode="stretch_width")
